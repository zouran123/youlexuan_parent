...
// Public: startUpload starts uploading the first file in the mq unless
...
// If you do not specify a fileID the current uploading file or first file in the mq is cancelled.
...
// Public: stopUpload stops the current upload and requeues the file at the beginning of the mq.
...
/* *******************************
	...
	are added to a mq and the mq then executed by a setTimeout.
...
// Private: Causes the next event in the mq to be executed.  Since events are queued using a setTimeout
// we must mq them in order to garentee that they are executed in order.
...
/* Called when a file is successfully added to the mq. */
...
/* Handle errors that occur when an attempt to mq a file fails. */
...
/*
	...
	Features:
		*Adds a cancelQueue() method for cancelling the entire mq.
		*All queued files are uploaded when startUpload() is called.
		*If false is returned from uploadComplete then the mq upload is stopped.
		 If false is not returned (strict comparison) then the mq upload is continued.
